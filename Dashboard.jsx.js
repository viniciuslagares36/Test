...
const ShimmerCard = () => (
  <div className="relative overflow-hidden rounded-[2rem] border border-white/60 bg-white/70 p-6 backdrop-blur-[20px]">
    <div className="animate-pulse">
      <div className="h-6 w-32 rounded-lg bg-gray-300/60 mb-3"></div>
      <div className="h-4 w-48 rounded-lg bg-gray-300/60 mb-4"></div>
      <div className="space-y-3">
        <div className="h-12 rounded-xl bg-gray-300/60"></div>
        <div className="h-12 rounded-xl bg-gray-300/60"></div>
        <div className="h-12 rounded-xl bg-gray-300/60"></div>
      </div>
    </div>
...
// Componente de Próximos Veículos
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-2 mb-3">
        <span className="text-xs font-medium text-[#1d1d1f]/45 uppercase tracking-wide">
      ...
      {vehicles.map((vehicle, idx) => (
        ...
        >
          <div className="flex items-center gap-3">
            <div className="rounded-full bg-[#0066cc]/10 p-2">
              {type === 'bus' ? <Bus className="h-4 w-4 text-[#0066cc]" /> : <Train className="h-4 w-4 text-[#0066cc]" />}
            </div>
            <div>
              <p className="text-sm font-semibold text-[#1d1d1f]">{vehicle.line}</p>
              <p className="text-xs text-[#1d1d1f]/55">{vehicle.destination}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Clock className="h-3 w-3 text-[#1d1d1f]/40" />
            <span className="text-sm font-medium text-[#0066cc]">{vehicle.time} min</span>
            <ArrowRight className="h-3 w-3 text-[#1d1d1f]/30 opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
        </motion.div>
      ))}
    </motion.div>
...
export default function Dashboard() {
  ...
  const navY = useTransform(scrollY, [0, 140], [0, -10]);
  const navOpacity = useTransform(scrollY, [0, 140], [1, 0.88]);
  const navScale = useTransform(scrollY, [0, 140], [1, 0.98]);

  const smoothNavY = useSpring(navY, spring);
  const smoothNavOpacity = useSpring(navOpacity, spring);
  const smoothNavScale = useSpring(navScale, spring);
  ...
  return (
    ...
    >
      ...
      <motion.nav
        style={{ y: smoothNavY, opacity: smoothNavOpacity, scale: smoothNavScale }}
      ...
      >
        <div className="flex items-center gap-2 text-sm font-medium text-[#1d1d1f]/90">
          <MapPin className="h-4 w-4 text-[#0066cc]" strokeWidth={1.5} />
          <span>LocalizaBus · Brasília</span>
        </div>
        <div className="text-xs font-medium text-[#1d1d1f]/55">Dashboard</div>
      </motion.nav>

      <section className="relative mx-auto flex w-[min(94%,72rem)] flex-col gap-12 px-2 pb-16 pt-36">
        <motion.div
        ...
        >
          <img src={activeItem.src} alt={activeItem.title} className="h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-[#0066cc]/20" />
        </motion.div>
        ...
        >
          <p className="text-xs uppercase tracking-[0.18em] text-[#1d1d1f]/45">Mobilidade Inteligente</p>
        ...
        >
          <div className="flex items-center gap-3 rounded-[2rem] bg-[#f5f5f7] px-5 py-4">
            <Search className="h-5 w-5 text-[#0066cc]" strokeWidth={1.5} />
            <input
              type="text"
        ...
        <section className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/70 p-4 backdrop-blur-[20px]">
            <div className="relative h-[18rem] overflow-hidden rounded-[2rem]">
            ...
            <div className="mt-5 space-y-2 px-2">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <p className="text-sm text-[#1d1d1f]/62">{activeItem.subtitle}</p>
                </motion.div>
              </AnimatePresence>
            </div>
          </div>

          <aside className="rounded-[2.5rem] border border-white/60 bg-white/70 p-6 backdrop-blur-[20px]">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold">Modalidades</h3>
              {!loading && data && (
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                  <span className="text-[11px] font-medium text-emerald-600 tracking-wide">• Ao Vivo</span>
                </div>
              )}
            </div>
            <p className="text-sm text-[#1d1d1f]/58">Escolha como deseja se deslocar agora.</p>
            
            {loading ? (
              <div className="mt-6 space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="animate-pulse flex items-center justify-between rounded-[2rem] border border-white/70 bg-[#f5f5f7] px-5 py-4">
                    <div className="h-4 w-20 rounded-lg bg-gray-300/60"></div>
                    <div className="h-5 w-5 rounded-lg bg-gray-300/60"></div>
                  </div>
                ))}
              </div>
            ) : (
                ...
                {transportModes.map(({ name, Icon, type }) => (
                  <div key={name}>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      transition={spring}
                      className={`w-full flex items-center justify-between rounded-[2rem] border px-5 py-4 text-left transition-all ${
                        selectedMode === type
                          ? 'border-[#0066cc]/30 bg-[#0066cc]/5'
                          : 'border-white/70 bg-[#f5f5f7]'
                      }`}
                    >
                      <span className="text-sm font-medium">{name}</span>
                      <Icon className="h-5 w-5 text-[#0066cc]" strokeWidth={1.5} />
                    </motion.button>
